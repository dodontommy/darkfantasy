...
depends_on: []
status: done
started_at: 20260510T061311Z
log_path: orchestrator/logs/cursor-mcp-example-cursor-mcp-pose-tweak-20260510T061311Z.log
finished_at: 20260510T061558Z
duration_s: 167
exit_code: 0
---
...
**2026-05-10 worker:** `localhost:9876` is listening but Blender MCP RPC returns
no data (addon not connected). `pgrep` shows no `blender` GUI process—only
`uvx blender-mcp` bridges. `get_scene_info` / viewport screenshot unavailable;
`outputs/renders/pose_threequarter_v1.png` not produced. Re-run after starting
GUI Blender with the blockout loaded and the MCP addon connected.
