...
class BlenderMCPServer:
    def __init__(self, host=None, port=None):
        # PATCH (darkfantasy): env-driven host/port so the addon can bind to
        # 0.0.0.0 without rebuilding. Constructor args still win when set
        # explicitly by the operator (which passes scene props).
        self.host = host or os.environ.get('BLENDER_MCP_HOST', '127.0.0.1')
        self.port = int(port if port is not None else os.environ.get('BLENDER_MCP_PORT', 9876))
        self.running = False
        self.socket = None
...
class BLENDERMCP_PT_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # PATCH (darkfantasy): host field exposes the bind address. Set 0.0.0.0
        # to accept LAN connections (useful with Tailscale + socat on a remote
        # host running the MCP server). Default 127.0.0.1 keeps the original
    ...
    def execute(self, context):
        ...
        if not hasattr(bpy.types, "blendermcp_server") or not bpy.types.blendermcp_server:
            bpy.types.blendermcp_server = BlenderMCPServer(
                host=scene.blendermcp_host, port=scene.blendermcp_port,
            )
...
def register():
    # PATCH (darkfantasy): host StringProperty seeded from BLENDER_MCP_HOST env
    # so launchers/scripts can override without UI clicks. Use 0.0.0.0 to
    # expose on all interfaces (LAN/Tailscale).
    from bpy.props import StringProperty
    bpy.types.Scene.blendermcp_host = StringProperty(
        name="Host",
        description="Bind address. 127.0.0.1 = localhost only; 0.0.0.0 = all interfaces (LAN exposure).",
...
def unregister():
    ...
    del bpy.types.Scene.blendermcp_host  # PATCH (darkfantasy)
