...
def get_viewport_screenshot(ctx: Context, max_size: int = 800) -> Image:
    ...
    """
    # PATCH (darkfantasy): cross-host screenshot. The upstream code constructs
    # a temp path on the SERVER host (Linux) and sends it to the addon to
    # write — but the addon may live on a remote host (Windows over an SSH
    # tunnel) where /tmp does not exist. Instead, the patched addon writes
    # to its own local temp, base64-encodes the bytes, and returns them in
    # the response. The server only handles bytes.
    import base64
    try:
        blender = get_blender_connection()
        ...
        # New protocol: addon returns base64-encoded bytes.
        if isinstance(result, dict) and result.get("data_b64"):
            image_bytes = base64.b64decode(result["data_b64"])
            return Image(data=image_bytes, format="png")

        # Backward-compat: older addon wrote to a server-supplied filepath.
        # Re-issue with explicit filepath under server-local temp and read it.
        ...
        if not os.path.exists(temp_path):
            raise Exception(
                "Screenshot file was not created. If the addon is on a remote "
                "host, install the patched addon from "
                "patches/blender-mcp/addon.py — the upstream addon cannot do "
                "cross-host screenshots."
            )
